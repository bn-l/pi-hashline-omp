...
`SWAP.BLK N:` — replace the whole syntactic block that BEGINS on line N; the block resolver determines the closing line. Body rows below.
...
- Whole construct → `SWAP.BLK N` (block resolver determines the end); lines inside it → `SWAP N.=M`.
